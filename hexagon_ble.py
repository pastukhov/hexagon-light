    ...
    for s in svcs:
        ...
        for c in s.characteristics:
            ...
            if "write" in props or "write-without-response" in props:
                candidates.append(
                    CandidateChar(uuid=c.uuid, props=props, service_uuid=s.uuid)
                )

    print("\n=== Writable characteristic candidates ===")
    for i, c in enumerate(candidates):
        print(
            f"[{i}] {c.uuid}  props=[{_fmt_props(c.props)}]  (service={c.service_uuid})"
        )
...
async def try_enable_notifications(client: BleakClient) -> None:
    ...
    async def _handler(sender: int, data: bytearray):
    ...
    for s in svcs:
        for c in s.characteristics:
            ...
            if "notify" in props or "indicate" in props:
                print(
                    f"  notify char: {c.uuid} props=[{_fmt_props(props)}] (service={s.uuid})"
                )
                try:
...
async def main() -> int:
    ...
    async with BleakClient(mac, timeout=15.0) as client:
        ...
        chosen: Optional[CandidateChar] = None
        ...
        print(
            f"\nUsing writable characteristic: {chosen.uuid} (service={chosen.service_uuid}, props={chosen.props})"
        )
        ...
        while True:
            ...
            try:
                ...
                await client.write_gatt_char(
                    chosen.uuid, payload, response=not without_response
                )
                print(
                    f"[WRITE] {chosen.uuid} <= {payload.hex()} (response={not without_response})"
                )
            except Exception as e:
