...
class HexagonLight:
  ...
  async def _connect_async(self) -> None:
    ...
    for attempt in range(1, self._connect_retries + 1):
      ...
      try:
        client = BleakClient(self.address, timeout=self._timeout, disconnected_callback=self._on_disconnect)
        ...
        if svcs is not None and hasattr(svcs, "get_service"):
          ...
          if ch is None:
            await client.disconnect()
            raise HexagonLightError(f"Write characteristic not found: {self._write_uuid}")
        ...
        with self._state_lock:
          self._client = client
          self._write = _WriteConfig(service_uuid=self._service_uuid, char_uuid=self._write_uuid, response=response)
          self._notify_event.clear()
        return
    ...
    raise HexagonLightError(f"Failed to connect after {self._connect_retries} attempts: {last_err}")
...
def _main() -> int:
  parser = argparse.ArgumentParser(description="Control Hexagon Light over BLE (MeRGBW protocol).")
  parser.add_argument("--mac", default="FF:FF:11:52:AB:BD", help="BLE MAC address (default: %(default)s)")
  parser.add_argument("--wait", type=float, default=0.0, help="If >0, wait this many seconds and print status")
